...
// import Buffer from "../config/esbuild.inject.js";
...
// console.log(JSON.parse(formData.source_code.toString()))
...
//     console.log(process.env.REACT_APP_RAPID_API_HOST);
    ...
    axios
      ...
      .catch((err) => {
        ...
        let error = err.response ? err.response.data : "sthg went wrong";
        //get error status
  ...
  const checkStatus = async (token) => {
    ...
    const options = {
      ...
      headers: {
        "X-RapidAPI-Host": process.env.REACT_APP_RAPID_API_HOST,
        // "X-RapidAPI-Key": process.env.REACT_APP_RAPID_API_KEY,
        "X-RapidAPI-Key" : "6f7d128189mshe5db813ed435fbep1c970djsna72a4c91f584",
    ...
    try {
      ...
      console.log('get status', response.data.status?.id)
      ...
      if (statusId === 1 || statusId === 2) {
        ...
        setProcessing(false);
        console.log("get response processed", response.data)
        setOutputDetails(response.data);
        // setOutputDetails(binaryAgent3(response.data));

        showSuccessToast(`Compiled Successfully!`);
        console.log("get response finished", response.data);
        return;
      }
    } catch (err) {
      console.log("err", err);
